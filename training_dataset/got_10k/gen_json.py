...
def check_borders(frame_sz, bbox):
    ...
    ok = (bbox[0] > dist_from_border) and (bbox[1] > dist_from_border) and ((frame_sz[0] - bbox[2]) > dist_from_border) and ((frame_sz[1] - bbox[3]) > dist_from_border)
    return ok
...
for subset in got10k:
    ...
    frames = subset['frame']
    snippet = dict()
    snippets[subset['base_path']] = dict()
    for f, frame in enumerate(frames):
        ...
        if bbox[2] <= 0 or bbox[3] <= 0 or bbox[0] < 0 or bbox[1] < 0 or (bbox[0] + bbox[2]) > frame_sz[0] or (bbox[1] + bbox[3]) > frame_sz[1]:
            count += 1
            print("count, [w, h], [x_min, y_min, x_max, y_max], frame_sz: ", count, [bbox[2], bbox[3]], [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]], frame_sz)
            continue

        snippet['{:06d}'.format(f)] = [bbox[0], bbox[1], bbox[0] + bbox[2], bbox[1] + bbox[3]]  # (xmin, ymin, xmax, ymax)

    snippets[subset['base_path']]['{:02d}'.format(0)] = snippet.copy()
